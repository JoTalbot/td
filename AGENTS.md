...
| `scripts/WeatherFX.gd` | Визуальная погода без боевых модификаторов: ясный зной, пыльный фронт, пеплопад; частицы пепла только при `Junk.quality_high` |
...
- Раскраски корпуса: `Campaign.truck_paint`, доступность через `available_paints()`, применение только `Truck.apply_paint_scheme()`. Предпросмотр обязан получать тот же paint id.
