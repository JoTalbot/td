...
- Именованные трассы: метаданные в `CampaignData.ROUTE_META`, ключ только через `route_key()` (сортированный `a|b`); `modifier` применяется в `Main._apply_campaign_effects()`, событие — ветка в `RoadEvents.trigger()`. `route_mastery` растёт только в `_on_run_completed()`, каждые 2 проезда дают уровень (макс.3): −5% опасности и +5% награды. На уровне 3 ключ пишется в `mastered_routes`, награда и скидка выдаются один раз.
- Война дорог: `route_control[route_key] = city_id`; `advance_faction_war()` тикает после каждого исхода рейса. Цвет контроля рисует `MapCanvas`, боевой эффект зависит от `rep_level()` контролирующего города.
- Услуги городов: `CITY_SERVICES` + `Campaign.buy_city_service()`; в `service_buffs` хранить `{id, strength}`. Финал истории `story_ending()` (`allied/mercenary/betrayed`) меняет цену/силу услуги, число контрактов и цвет кольца `CityMarker`. Выборы сохраняются в `story_choices` (`loyal/profit/betray`).
- Туман войны: `Campaign.discovered_cities`; новый профиль видит город и соседей, старый сейв без поля мигрирует с полностью открытой картой. Контракт `scout` открывает цель, усиливает рейс и ставит `WaveManager.scout_boss`; финальная волна должна содержать ровно одного `scoutboss`.
- Достижения: данные в `CampaignData.ACHIEVEMENTS`, условия централизованы в `Campaign.check_achievements()`; награда выдаётся ровно один раз. Счётчики серий живут в `achievement_stats`, сигналы `achievement_unlocked`/`route_mastered` используют одну очередь плашек HUD.
