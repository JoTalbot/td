...
- Именованные трассы: метаданные в `CampaignData.ROUTE_META`, ключ только через `route_key()` (сортированный `a|b`); `modifier` применяется в `Main._apply_campaign_effects()`, гарантированное событие — одноимённая ветка в `RoadEvents.trigger()`.
- Услуги городов: `CITY_SERVICES` + `Campaign.buy_city_service()`; в `service_buffs` хранить `{id, strength}`, чтобы ранг репутации фиксировался при покупке. Сюжеты: `CITY_STORIES`, прогресс `story_progress`, арт `art_bone_1..3` / `art_copper_1..3`.
- Туман войны: `Campaign.discovered_cities`; новый профиль видит город и соседей, старый сейв без поля мигрирует с полностью открытой картой. Разведданные открывают только следующий пограничный город.
- Достижения: данные в `CampaignData.ACHIEVEMENTS`, условия централизованы в `Campaign.check_achievements()`; награда выдаётся автоматически ровно один раз.
