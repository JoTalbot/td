...
4. Держать производительность мобильной: рендерер `mobile`, тени только от солнца. Любая дорогая детализация обязана иметь ветку `Junk.quality_high`; экономный профиль не должен создавать дополнительные меши, glow, дымовой слой и OmniLight.
...
| `scripts/TruckPreview.gd` | SubViewport Шоурума: текущий корпус и косметика; автовращение, drag и pinch-зум без влияния на боевую сцену |
...
| `scripts/WeatherFX.gd` | Погода: ясный зной, пыльный фронт, пеплопад; сигнал `changed` задаёт отдельные `weather_*` множители GameState, пепельные частицы только при `Junk.quality_high` |
| `scripts/PhotoMode.gd` | Фоторежим поверх паузы: скрывает HUD, оставляет CameraRig в PROCESS_ALWAYS, переключает погоду и сохраняет PNG в `user://` |
...
- Именованные трассы: `route_mastery` растёт только в `_on_run_completed()`. На уровне 3 ключ пишется в `mastered_routes`, один раз выдаются лом/запчасти/чертёж и скидка −3% (потолок −18%). Косметика хранится в `Campaign.route_cosmetics`, переключается только через `toggle_route_cosmetic()` и строится `Truck.apply_route_cosmetics(mastered_count, enabled)` после каждой смены корпуса.
- Война дорог: `route_control[route_key] = city_id`, срок — `route_control_age`; `advance_faction_war()` тикает после каждого исхода рейса и пишет максимум 20 записей в `war_log`. Командир из `FACTION_COMMANDERS` появляется через `route_commander()` только после нужного срока удержания. Новый командир требует: тип в WaveManager, визуал/фазы в Enemy, запись TROPHIES, `t_*.png` и рецепт Кузни. Экран `front` показывает гарнизоны, таймер и прогноз хода. Контратаки: смена владельца ставит `counterattacks[key] = {by, owner, eta 2–3 рейса}`; тик в начале `advance_faction_war()` (удар возвращает дорогу, запись `{counter: true}`); оборона — `Campaign.repel_counterattack()` из `Main._on_run_completed()` (+120 лома, +3 реп, запись `{repelled: true}`); `Main._on_travel()` усиливает такой рейс (+2 рейдера/волну, командир атакующей фракции, если слот свободен).
- Недельная кампания хранится в `war_week/war_side/war_points/war_claimed`, сбрасывается по Unix-неделе и выдаёт цели 4/8/15 один раз.
...
- Графические профили: `UserSettings.effects` (`full/economy`) → `Junk.quality_high`. Дополнительные меши, протектор, погода, glow, дым, фары, трассеры и кинематографическая камера обязательны только в качественном профиле; смена профиля на карте перезагружает сцену.
- Раскраски корпуса: `Campaign.truck_paint`, доступность через `available_paints()`, применение только `Truck.apply_paint_scheme()`. Предпросмотр обязан получать тот же paint id. Темы UI хранятся в `UserSettings.ui_theme`, применяются при пересборке сцены; гейтинг задаёт MapScreen по сюжетам/мастерству.
...
- Караванный Казначей (трасса 🐫): `WaveManager.treasurer_planned` (ставит `Main._on_travel`) → `_spawn_treasurer()` на средней волне; в `enemies_alive` НЕ входит — волну не блокирует ни убитый, ни сбежавший. Поведение `Enemy.fleeing`: дуга `_plan_escape()`, паника <50% HP (`_check_flee_panic`: балласт `{parts,fuel}` через `cargo_dropped` → `WaveManager.cargo_looted` → `Main._on_cargo_looted()`, скорость ×1.55). Убитый: трофей с шансом 1.0 + джекпот-груз в `Main._on_enemy_killed()`; сбежавший — сигнал `escaped` → анонс. Рецепт Кузни: `leg_grabcrane`.
