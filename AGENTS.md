...
- Именованные трассы: `route_mastery` растёт только в `_on_run_completed()`. На уровне 3 ключ пишется в `mastered_routes`, один раз выдаются лом/запчасти/чертёж и постоянная скидка −3% (общий потолок −18%).
- Война дорог: `route_control[route_key] = city_id`; `advance_faction_war()` тикает после каждого исхода рейса. Недельная кампания хранится в `war_week/war_side/war_points/war_claimed`, сбрасывается по Unix-неделе и выдаёт цели 5/12/25 один раз.
