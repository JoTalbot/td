...
| `scripts/TruckPreview.gd` | SubViewport Шоурума: вращаемый корпус с текущей косметикой мастерства, без влияния на боевую сцену |
...
- Война дорог: `route_control[route_key] = city_id`, срок — `route_control_age`; `advance_faction_war()` тикает после каждого исхода рейса и пишет максимум 20 записей в `war_log`. Командир из `FACTION_COMMANDERS` появляется через `route_commander()` только после нужного срока удержания. Экран `front` показывает все гарнизоны.
