...
- Война дорог: `route_control[route_key] = city_id`; `advance_faction_war()` тикает после каждого исхода рейса и пишет максимум 20 записей в `war_log`. Недельная кампания хранится в `war_week/war_side/war_points/war_claimed`, сбрасывается по Unix-неделе и выдаёт цели 5/12/25 один раз.
